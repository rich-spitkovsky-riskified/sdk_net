...
namespace Riskified.SDK.Sample
{
    public static class OrderTransmissionExample
    {
            ...
            while (commandStr != null && (!commandStr.Equals("q")))
            {
                ...
                try
                {
                    ...
                    switch (commandStr)
                    {
                        ...
                        case "ad":
                            Console.WriteLine("Order Advise Decide Generated with merchant order number: " + orderNum);
                                var orderAdviseDecideCheckout = GenerateAdviseDecideOrderCheckout(orderNum.ToString());
                            orderAdviseDecideCheckout.Id = orderNum.ToString();
                            adRes = gateway.AdviseDecide(orderAdviseDecideCheckout);
                            break;
                    ...
                    if (adRes != null)
                    {
                        Console.WriteLine("\nAdvise Decide sent successfully:" +
                            "\nStatus at Riskified:" + adRes.Status +
                            "\nOrder ID received:" + adRes.Id +
                            "\nDescription: " + adRes.Description +
                            "\nAuthenticationType: " + adRes.AuthenticationType.AuthType +
                            "\nScore: " + adRes.Score);
                    }
        ...
        private static OrderBase GenerateAdviseDecideOrderCheckout(string orderNum)
        {
            var orderCheckout = new OrderCheckout(orderNum);
